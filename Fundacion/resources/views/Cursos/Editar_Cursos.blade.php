...
<form class="mt-3 w-75 needs-validation" novalidate method="post" action="{{route('Cursos.update', $item->idCurso);}}">
    ...
    <div class="mb-3">
        ...
        <input name="Nombre" type="text" class="form-control" id="Nombre" required value="{{$item->Nombre}}">
        @if ($errors->has('Nombre'))
          <div>*El campo Nombre no puede se null</div><br>
        @endif
      </div>
      <div class="form-floating">
        <textarea name="Descripcion" class="form-control" placeholder="Leave a comment here" id="floatingTextarea2" style="height: 200px" required>{{$item->Descripcion}}</textarea>
      ...
      <div class="mb-3">
        ...
        <select class="form-select" name="Categoria" aria-label="Default select example">
          @foreach ($arrayCategoria as $item2)
              @if ($item->idCategoria == $item2->idCategoria)
                <option value="{{$item->idCategoria}}"selected>{{$item2->Nombre}}</option>
              @else
                <option value="{{$item2->idCategoria}}">{{$item2->Nombre}}</option>
              @endif
          @endforeach
        </select>
    </div>
