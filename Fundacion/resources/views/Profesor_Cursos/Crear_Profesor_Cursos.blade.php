...
<div class="row mt-5">
    <div class="d-flex justify-content-center">
        <h1>ALTA DE PROFESORES X CURSOS</h1>
    </div>
</div>
<div class="row mt-5">
<div class="d-flex justify-content-center">
<form class="mt-3 w-75 needs-validation" novalidate method="post" action="{{route('Profesor_Cursos.store');}}">
    @csrf
    <div class="mb-3">
        ...
        <option selected>Seleccionar Profesor</option>
          @foreach ($arrayPersonalCursos as $item)
            <option value="{{$item->idPersonal}}">{{$item->Nombre}} {{$item->Apellido}}</option>
          @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <option selected>Seleccionar Curso</option>
          @foreach ($arrayDatosCursos as $item)
